...
This will run through all the questions, verifying that you either
correctly answered or skipped all of them.

Next, you should commit and sync the following files:
...
Q. List the URI, English label and number of swamps for all the
counties of North Carolina. The list should be ordered by number of
...
constructs new triples based on the triples in `description.ttl`,
which are written using the [Schema.org](https://schema.org)
vocabulary.

The new triples will use properties from the [Dublin Core Metadata
Initiative (DCMI) Metadata
Terms](https://www.dublincore.org/specifications/dublin-core/dcmi-terms/)
instead of Schema.org.

Look at the DCMI Metadata Terms (specifically, the properties in the
`/terms/` namespace), and find two or three properties that seem
similar in meaning to properties used in `description.ttl`. Then write
a `CONSTRUCT` query that constructs new triples using the DCMI
properties.
